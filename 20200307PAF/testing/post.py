...
from data_process.coco_process_utils import BODY_PARTS, POINTS
from data_process.process_utils import image_cropped
...
def decode_pose(img_orig, param, heatmaps, pafs):
    ...
    #a NUM_JOINTS(multiple people included) x 5 np.array where each row represents a joint type (0=nose, 1=neck...)
    #and the columns indicate the {x,y} position, the score (probability)， a unique id (counter) and the joint_type(0=..., 1=...)
    '''For example, for 2 people, len=21
    joint_list[[3.27000000e+02 6.70000000e+01 1.43553913e-01 0.00000000e+00
  0.00000000e+00]
 [2.39000000e+02 1.10000000e+02 7.10410893e-01 1.00000000e+00
  0.00000000e+00]
 .
 .
 .
 [2.35000000e+02 1.08000000e+02 7.76678860e-01 2.00000000e+01
  1.20000000e+01]]
    '''
    joint_list = np.array([tuple(peak) + (joint_type,) for joint_type,
                                                           joint_peaks in enumerate(joint_list_per_joint_type) for peak
                           in joint_peaks])
#    print('joint_list.shape={}'.format(joint_list.shape))
#    print('joint_list{}'.format(joint_list))
    ...
    person_to_joint_assoc[[ 1.          3.          4.          5.          6.          7.
   9.         11.         12.         14.         16.         17.
  20.         12.11059102 13.        ]
 [-1.         -1.         -1.         -1.         -1.         -1.
   8.         10.         -1.         -1.         -1.         -1.
...
def append_result(img_basic, image_id, person_to_joint_assoc, joint_list, outputs):
    ...
    for ridxPred in range(len(person_to_joint_assoc)):
        ...
        if len(joint_list) != 0:
            ...
            for part in range(0,NUM_JOINTS*2, 2):
                ind = part//2
                ...
                if -1 == index:
                    ...
                    keypoints[part] = joint_list[index, 0] + 0.5
                    keypoints[part + 1] = joint_list[index, 1] + 0.5
            crop_location, cropped_image = image_cropped(img_basic, keypoints)
            cropped_image = cropped_image.tolist()
            one_result["crop locationWH"], one_result["cropped image"] = crop_location, cropped_image
            one_result["keypoints"] = list(keypoints)
